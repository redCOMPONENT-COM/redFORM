...
# move redform package to right place
cd /tests/www
mkdir tests
mkdir repo
cd $WORKSPACE
cd $(pwd)/tests/joomla-cms
sed -i "s/$db = 'redformSetupDb'/$db = '${dbName}${STAGE}${fetch}'/g" configuration.php
...
mv redform.zip /tests/www/tests/releases/redform.zip
