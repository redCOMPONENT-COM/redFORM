...
window.addEvent('domready', function() {
	$('cr_function').addEvent('change', function(){
		var span = $("cr_params");
		span.empty();
	...
	$('cr_button').addEvent('click', function(){
		var line = '';
		
		// reset error status
		$('cond_recipients_ui').getElements('input').each(function(element){
			element.removeClass('error');
		});
		$('cond_recipients_ui').getElements('select').each(function(element){
			element.removeClass('error');
		});
		
		if (!rfConditionalRecipient.checkEmail($("cr_email").value)) {
			alert(emailrequired);
