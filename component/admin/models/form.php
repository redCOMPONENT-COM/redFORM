...
class RedformModelForm extends JModel 
{
  ...
  function store($post)
  {
  	...
  	/* Get the posted data */
  	if (!$row->bind($post)) {
  		$this->setError(JText::_('There was a problem binding the form data'), 'error');
  		return false;
  	}
  	...
  	/* pre-save checks */
  	if (!$row->check()) {
  		$this->setError(JText::_('There was a problem checking the form data'), 'error');
  		return false;
  	}

  	/* save the changes */
  	if (!$row->store()) {
  		$this->setError(JText::_('There was a problem storing the form data'), 'error');
  		return false;
  	}
  ...
  function copy($cids = array())
  {
    foreach ($cids as $cid)
    {
      ...
      // copy the form
	    $form->id = null;
      $form->formname =  JText::_('Copy of') .' '. $form->formname;
      $form->store();      
