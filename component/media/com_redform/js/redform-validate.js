 ...
 *
 * Inspired by: Chris Campbell <www.particletree.com> and Joomla Framework
 *
 * @package Redform
 * @since 2.5
 */
var RedformFormValidator = new Class({
	initialize: function()
	{
		...
		var subforms = $$('.formbox');
		subforms.each(function(subform){ this.attachToSubform(subform); }, this);
	},
	...
	attachToSubform: function(subform)
	{
		// Iterate through the form object and attach the validate method to all input fields.
		subform.getElements('input,textarea,select,button').each(function(el){
			...
			if ((document.id(el).get('tag') == 'input' || document.id(el).get('tag') == 'button') && document.id(el).get('type') == 'submit') {
				if (el.hasClass('validate')) {
					el.onclick = function(){return document.redformvalidator.isValid(this.subform);};
				}
			} else {
				el.addEvent('blur', function(){return document.redformvalidator.validate(this);});
...
window.addEvent('domready', function(){
	document.redformvalidator = new RedformFormValidator();
});
