...
class RedFormModelPayment extends JModel
{
	 ...
	 *
	 * @param   boolean  $filter_language  filter by language
	 *
	 * @return array
	 */
	public function getGateways($filter_language = false)
	{
		if (empty($this->_gateways))
		{
			if ($filter_language && JFactory::getApplication()->getLanguageFilter())
			{
			...
			$gateways = array();
			$results = $dispatcher->trigger('onGetGateway', array(&$gateways, $lang));
	...
	{
		// For the select, filter according to language
		$gw = $this->getGateways(true);
	...
	/**
	 * returns submitter for current key
	 *
	 * @return array
	 */
	 ...
	 * @return unknown_type
	 */
	public function getPaymentDetails($key)
	{
		...
		switch ($asub->integration)
		{
			case 'redevent':
				...
				$obj->title = JText::_('COM_REDFORM_Event_registration') . ': '
					. $event->title . ' @ ' . $event->venue . ', ' . strftime('%x', strtotime($event->dates)) . ' '
					. ($event->times && $event->times != '00:00:00' ? $event->times : '');
	 ...
	 * @deprecated
	 */
	public function getEventAttendee($key)
	{
	 ...
	 * @return void
	 */
	protected function _notifySubmitter()
	{
	 ...
	 * @return void
	 */
	protected function _notifyFormContact()
	{
		$mainframe = &JFactory::getApplication();
		$mailer = &JFactory::getMailer();
		$mailer->From = $mainframe->getCfg('mailfrom');
